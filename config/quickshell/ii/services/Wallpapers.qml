...
Singleton {
    ...
    function apply(path, darkMode = Appearance.m3colors.darkmode) {
        ...
        Quickshell.execDetached(["bash", "-c", `echo "[Wallpapers] APPLYING: ${path}" >> /tmp/quickshell_debug`]);
        
        // Try to apply as a curated theme first
        if (ThemeManager.applyThemeForWallpaper(path, darkMode)) {
            Quickshell.execDetached(["bash", "-c", `echo "[Wallpapers] Handled by ThemeManager" >> /tmp/quickshell_debug`]);
            root.changed()
            return
        }

        Quickshell.execDetached(["bash", "-c", `echo "[Wallpapers] Handled by switchwall.sh" >> /tmp/quickshell_debug`]);
        applyProc.exec([
            Directories.wallpaperSwitchScriptPath,
            "--image", path,
            "--mode", (darkMode ? "dark" : "light")
